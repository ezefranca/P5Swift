...
- Keep Metal source warning-free for both macOS and iOS Simulator SDKs. Do not hide a backend change behind silent fallback.
...
If the package includes an application sample, format it and let the validator build both configurations. Instruments traces are local diagnostics; summarize findings in release notes and never commit trace archives.
